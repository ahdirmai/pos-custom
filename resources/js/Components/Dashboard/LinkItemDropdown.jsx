    ...
    return (
        <div className="w-full">
            <button
                ...
                className={`min-w-full flex items-center font-medium gap-x-3.5 px-4 py-2.5 transition-all duration-200 text-sm justify-between ${hasActiveChild ? 'bg-primary-50 text-primary-700 border-l-[3px] border-primary-500' : 'text-slate-600 hover:bg-slate-100 border-l-[3px] border-transparent'
                    }`}
            >
            ...
            {isOpen && sidebarOpen && data.map((item, i) => (
                item.permissions && (
                    <Link
                        key={i}
                        href={item.href}
                        className={`min-w-full flex items-center gap-x-3.5 px-5 py-2.5 text-sm transition-all ${checkIsActive(item) ? 'bg-primary-100 text-primary-800 border-l-[3px] border-primary-600' : 'text-slate-600 hover:bg-slate-50'
                            }`}
                    >
