...
<head>
    ...
    <style>
        ...
        .customer-name { font-size: 12pt; font-weight: 800; margin-bottom: 2px; }
...
<body>
                    ...
                    <table style="border:none;">
                        <tr>
                            <td class="header-logo">
                                @if ($store['logo'] ?? false)
                                    <img src="{{ $store['logo'] }}" style="max-width:45pt;">
                                @else
                            ...
                            <td class="company-details">
                                <h1>{{ $store['name'] }}</h1>
                                <p>{{ $store['address'] }}</p>
                                <p>Telp: {{ $store['phone'] }} @if($store['email']) • {{ $store['email'] }} @endif</p>
                            </td>
                        </tr>
                    </table>
                </td>
                <td class="text-right" style="vertical-align: top;">
                    <h1 class="doc-title">PIUTANG</h1>
                    <div class="doc-number">NO: {{ $receivable->invoice }}</div>
                    <div class="muted">Tempo: {{ $receivable->due_date ? \Carbon\Carbon::parse($receivable->due_date)->format('d/m/Y') : '-' }}</div>
                </td>
            </tr>
        </table>
    ...
    <div class="info-grid">
        <div class="col col-left">
            <div class="box-title">Pelanggan / Customer</div>
            <div class="customer-name">{{ $receivable->customer->name ?? 'Umum' }}</div>
            <div class="muted">
        ...
        <div class="col col-right">
            <div class="financial-summary">
                <div class="summary-row">
                    ...
                    <span class="value">Rp {{ number_format($receivable->total, 0, ',', '.') }}</span>
                </div>
                ...
                <div class="summary-row" style="color: #16a34a;">
                    ...
                    <span class="value">(-) Rp {{ number_format($receivable->paid, 0, ',', '.') }}</span>
                </div>
                ...
                <div class="summary-row" style="margin-top: 5px;">
                    ...
                    <span class="value dept">Rp {{ number_format(max(0, $receivable->total - $receivable->paid), 0, ',', '.') }}</span>
                </div>
            </div>
        </div>
    </div>

    <div class="box-title">Riwayat Pembayaran</div>
    ...
    <div class="signature-section">
        ...
        <div class="sig-box">
            ...
            <div class="sig-space"></div>
            <div class="sig-line">{{ $receivable->customer->name ?? 'Pelanggan' }}</div>
        </div>
    </div>
        ...
        <table style="width: 100%; border:none;">
            <tr>
                ...
                <td class="text-right">
                    ...
                    @endif
                    <span style="margin-left: 8px; font-weight: 700;">{{ $receivable->invoice }}</span>
                </td>
            </tr>
        </table>
    </div>
